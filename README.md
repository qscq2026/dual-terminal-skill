...
**真实项目复盘驱动的大修。** 用双终端模式开发了一个五子棋游戏（~15 轮迭代），跑完复盘发现 v2 版在实际使用中的多个缺口。

- **Verifier 红线** — 永远不碰代码，碰过就不再是独立第三方，整个隔离模型的信任基础就没了（复盘真实事件）
- **审查三技术** — 针对"看到聚合数值 → 停止思考"的漏判模式，新增数值翻译→mock 语义审计→根因三层问三个强制步骤，必须产出具体内容才算做到（不是自我报告）
- **问题严重程度分级** — 每条发现标注 `[阻塞]` 或 `[细节]`，只有 `[阻塞]` 卡审查；新增 `track-blocker-streak.sh`，连续 2 轮以上无新增阻塞时倾向于结束循环
- **终局判断三问** — 判 `APPROVED` 前必须写出三个具体假设（最可能错哪里 / 是否疲劳 / 是否差不多行了），写不出不能判
- **回滚机制** — 新增 `checkpoint.sh`（Worker 改代码前自动 commit + tag）/ `rollback-help.sh`（只展示不自动执行）
- **任务版本管理** — 新增 `set-task.sh` 自动归档旧任务到 `task-history.md`，防止 `task.txt` 堆叠成"沙包"
- **next-round.sh** — 解决 `set-task.sh` 和 `reset.sh` 各管各的、单独用哪个都不对的协作漏洞
- **自动 git init** — `start-dual-terminal.sh` 在非 git 项目自动 `git init`（`DUAL_CLAUDE_NO_GIT_INIT=1` 跳过）
- **.gitignore 自愈** — 双重确保 `.dual-claude/` 被 gitignore，避免协调文件混进代码提交
- **等待升级** — `wait-for-status.sh` 新增 5 分钟升级提醒（`TIMEOUT_ESCALATE`，响铃不停止），三档超时：90s → 5min → 30min
- **"记录不是防线"** — 明确 `verifier-violation-log.txt` 的真正价值不在自我报告，在逼着下一轮带具体记忆去审查
