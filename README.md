...
│   └── round-brief.sh           # 本轮简报，由 wait-for-status.sh 在匹配成功时自动调用
├── README.md                    # 本文件
...
│
├── scripts/                         ← 从 scripts/ 整个目录复制（14 个脚本）
...
│   ├── min-iter.txt                ← 本次审查的迭代下限（硬约束，set-status.sh 在没到这个数之前拒绝写入 APPROVED）
...
# track-blocker-streak.sh  heartbeat.sh  round-brief.sh
```
...
### v3.5：Context，不是 Control

一次更长的真实压力测试（169 轮，FinERP 项目）之后，复盘暴露的问题跟前几次
不是同一类。上次（v3.3）的核心是"审查深度不够、防线设计不足"；这次的核心
是：**连已经写成主循环里字面意义强制命令的步骤，长会话里照样被系统性跳
过**——步骤 2.5（交叉核对上一轮报告）在 SKILL 文件里是一行显式的 `cat`
命令，复盘里写的是"基本没执行过"；`checkpoint.sh` 明明写在"改代码之前"这
个位置，实际是"跳到第 7 轮才第一次创建回滚点"；`worker-phase.txt` 停在
第 141 轮，空转到 169 轮从未更新。

**这次没有直接动手改，先花了几轮把方向想清楚**，因为最初几个直觉方案后来
都被否掉了，记下来是为了不让以后的自己重新踩一遍：

- 一开始的思路是"把强制步骤打包"（比如做一个 `round-start.sh` 把读取
  违规记录、读计划、checkpoint 全部塞进一次调用）——这个被否了，因为它
  解决的还是错误的那一层：模型要是决定这一轮不走脚本这条路，打包只是把
  N 个可以被跳过的独立调用，合并成 1 个可以被整体跳过的调用，跳过的
  概率不会因为打包而降低。
- 第二个思路是"检测＋提醒"（比如用项目文件的 mtime 交叉核对
  `worker-phase.txt` 是不是真的在及时更新，不一致就报警）——这个也被
  否了，因为复盘原文写得很清楚："用户反复提醒'要持续监控'"、"被骂后
  改好，过几轮又复发"——**人工提醒这条路已经在真实场景里被验证无效了**，
  再多一层脚本自动生成的提醒，只是把"用户发现"换成"脚本提醒用户去
  发现"，中间多了一层，强制力没有变化。
- 真正的转向来自一句话：**"context, not control"**。SKILL.md 里写的
  规则能不能生效，不取决于当初写得多"强制"，取决于这份信息此刻离模型
  的决策点有多远——169 轮之后，最初读到的规则物理上已经埋在很深的历史
  里，这是长上下文模型的正常表现（离当前决策点越远，召回权重越低），不
  是模型不认真。一直在"指令够不够强硬"上使劲，从一开始就点错了方向。

**唯一真正管用的落点**：整套系统里只有一处结构上不可能被跳过——
`wait-for-status.sh` 匹配成功的那一刻。不看这个输出，模型不知道该干什么，
循环走不下去，这是 169 轮里唯一一条被证明每轮都真的会发生的路径。新增
`scripts/round-brief.sh`，焊在这个唯一的出口上：匹配成功时，自动打印一份
从 `.dual-claude/` 现算的"本轮简报"——任务、当前里程碑、上一轮的 `[阻塞]`
问题、历史违规/漏判记录、连续无阻塞轮数、几条铁律。不是新增一个模型需要
"记得调用"的步骤，是让该被看到的信息，出现在它本来就必须读的那份输出里。

这跟之前所有"把规则写得更强硬"的尝试有本质区别：**不指望模型记得或回忆，
只要求它读眼前这一份**——这份东西每轮都不一样、都是刚现算出来的，不是
反复出现的静态提醒文案。也是因为这一点，`round-brief.sh` 里刻意没有写
"提醒你注意 xxx"这种说教式的话术，只呈现现算出来的事实（上一轮结论、还没
解决的 `[阻塞]` 条目、历史记录条数），怎么处理交给模型自己判断——这是
避免把它当"需要反复叮嘱的东西"。

**这个方向解决不了什么**：如果模型某几轮压根没有在走 `wait-for-status.sh`
这条链路——169 轮复盘里"前几轮在对话里空等，未用有界轮询"就是这种情况——
那不管这份简报设计得多好，都没有机会被打印出来，因为脚本根本没被调用。
这条路径解决的是"模型在用这条链路、但对链路之外的信息/动作会随会话变长而
遗忘"，解决不了"模型完全脱离了这条链路，自由发挥"。后者大概率要看宿主
环境有没有工具调用层面的拦截机制，不是 SKILL.md + 脚本这一层能焊死的边界。

### v3.6：写完 round-brief.sh 立刻被自己打脸

v3.5 上线没多久就被问了一句："念叨多了会不会也变成走形式？"——回头看
`round-brief.sh` 里"铁律，每轮都适用"那段，发现自己刚犯了本文档前面明确
批判过的错误：同一批文字，每轮原样重复，跟"记录漏判本身不是防线"、"无
变更轮不该喂固定文案，该喂真实变化的事实"这两条结论直接矛盾——写解决方案
的时候，没有用同一把尺子量自己的解决方案。

需要拆开看的是两件事：**"结构上不可能被跳过"解决的是模型会不会读到这段
输出；"内容每轮是否一样"决定的是读到之后会不会真的被当回事**。v3.5 只
解决了第一件，默认了第二件会自动成立，但没有理由这么默认——固定文案不管
写得多重要，重复多少轮之后都会被模式识别成背景噪音，这跟它是不是被强制
读到没关系。

修法：把静态提醒清单，换成只在检测到具体异常迹象时才出现的证据，没有异常
就不出现，出现本身就是信号：

- Worker 侧：不再提醒"记得 checkpoint"，改成对比 `checkpoint-count.txt`
  和 `iteration.txt`，数字对不上才出现"[!] checkpoint 记录数少于迭代
  数"；不再提醒"别写笼统声明"，改成 grep 上一轮提交里有没有真的出现
  "全流程正确执行"这类短语，命中才提醒。
- Verifier 侧：步骤 2.5 的"要记得交叉核对"，改成直接把最近两轮的
  `[阻塞]` 清单原文并排列出来，同一个问题在不在两份里，一眼看出，不用
  被动等提醒；"核实 Worker 报告是否属实"这句提醒，改成直接跑
  `git diff --stat HEAD`，diff 是空的但 Worker 说改了代码，矛盾自己摆在
  眼前。

**唯一没找到证据化办法、老实承认没解决的一条**：Verifier"永远不碰代码"
这条红线。要可靠判断"这次改动是不是 Verifier 自己碰的"，需要比一次 diff
更深的溯源（谁在什么阶段动的手），这里没做到。索性没有把它塞进
`round-brief.sh` 稀释掉，让它只在 `SKILL-verifier.md` 最显眼的位置完整
出现一次——这是承认没解决，不是含糊过去。

**这条证据本身也有依赖，不是万无一失**：`git diff --stat HEAD` 这条的
可靠性完全依赖 `checkpoint.sh` 严格按规定时机调用（改代码前一次，不二次
调用）。如果这个时机被打乱（比如改完代码后又 checkpoint 了一次），改动
会被吸收进那次提交，diff 显示为空——不代表没改，代表上游动作的时机出了
问题。这条证据要跟其他信号一起看，不能单独当结论用。

### v3.7：Verifier 比 Worker 更先乱节奏，而且是同一个模型在说服自己

FinERP 复盘之后有人问了一句：真正先松懈的通常是 Verifier，不只是随意压缩
循环轮数，还包括空转时给自己找一个自洽的收敛理由、审查变得潦草——而且它
对"审查"这件事本身的理解、对自己角色的定位，在长会话里都不稳定。这跟
"至少15轮被曲解成4轮够了"是同一类问题的更一般形式，而且指向一个更让人
不安的结论：

**v3.3 加的技术一到技术四、终局判断三问，本质上都是"要求 Verifier 写出更
具体的文字来自证"。但一个能写出具体、像那么回事的举证文字的模型，同样有
能力写出具体、像那么回事的自洽收敛理由。** 只要检查手段完全活在 Verifier
自己生成的文本里，就有被同一个模型的流畅修辞绕过的空间——这不是这个模型
在撒谎，很可能是真诚地说服了自己，跟 v3.6 发现的"固定提醒会被模式识别成
噪音跳过"是不同的坑，但同样致命。

这次分两类处理，不再假设"让它多写点什么"就够：

**能变成硬约束的，就不再靠说服。** 之前"LIMIT"只解析出一个上限，"至少15
轮"这种下限表述被当成软指标处理，用户原话里的下限约束被曲解成了可以提前
满足的上限。现在 `LIMIT`（上限）和新增的 `MIN_ITER`（下限）分开解析，
裸数字（"循环N次"不带"至少/最多"修饰词）视为下限=上限，不再默认可以提前
收工。更关键的是 `MIN_ITER` 不是写在 SKILL 文件里的又一条规则——
`set-status.sh` 在迭代数没到这个下限之前，会直接拒绝写入 `APPROVED`，
不写入、退出码非零，不给"这轮我觉得够了"这种理由留任何执行空间。这是这套
系统里少数几处不靠"写清楚、指望模型照做"，而是靠代码直接拒绝的地方——因为
"迭代数够不够用户要求的下限"是一个纯粹可数的事实，不需要判断力，就不该
交给判断力决定。

**没法变成硬约束的（审查潦草、角色定位漂移），退而求其次：不再让 Verifier
凭空写自证文字，让它先看到自己刚刚写下的记录。** `round-brief.sh` 现在会
把 Verifier 最近几轮实际写下的判断结果（不是提醒，是它自己的历史记录原文）
摆在新一轮判断之前——道理很朴素：编一个理由去说服一个空白的读者很容易，
编一个理由去说服一个刚刚看过自己前几轮记录、且新理由不能跟那些记录明显
矛盾的读者，难度不一样。这不保证有效，只是比让它在真空里自证要难绕过
一些。

**这条依然没有解决的部分**：审查本身够不够深入，终究没有一个客观可数的
指标能完全替代判断力——`git diff --stat` 能证伪"报告与代码不符"，能证伪
"完全没做"，证伪不了"做了，但做得潦草"。这条上限，写在这里，不假装用
这两个改动解决了。
...
14 个脚本里都用同一种方式定位共享目录：`脚本所在目录的上一级` + `.dual-claude`
（即约定脚本永远放在项目根下的 `scripts/` 子目录里，共享目录在项目根）。如果
要换成固定路径而不是"相对脚本位置"，需要**同时改全部 14 个脚本**，保持一致，
